import copy

sbox = []

def num2mat(num):
    arr = []
    for i in range(16):
        byte = (num >> (8 * (15 - i))) & 0xff
        if i % 4 == 0:
            arr.append([byte])
        else:
            arr[int(i / 4)].append(byte)
    return arr

def mat2num(mat):
    num = 0
    for i in range(15, -1, -1):
        num += mat[int(i / 4)][i % 4] * (16 ** ((15 - i) * 2))
    return num

xtime = lambda x : (((x << 1) ^ 0x1b if x & 0x80 else x << 1) & 0xff)


def gen_keys(key):
    rc = [0x8d, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36]
    keys = [(key)]
    for key_round in range(1, 11):
        new_key = copy.deepcopy(keys[key_round - 1])

        w3 = copy.deepcopy(new_key[3])  # Function G on w1
        tmp = w3[0]
        w3[0] = sbox[w3[1]] ^ rc[key_round]  # Sbox + switch + RCON
        w3[1] = sbox[w3[2]]
        w3[2] = sbox[w3[3]]
        w3[3] = sbox[tmp]

        for i in range(4):
            new_key[0][i] = new_key[0][i] ^ w3[i]

        for i in range(1, 4):
            for j in range(4):
                new_key[i][j] ^= new_key[(i - 1)][j]

        keys.append(new_key)
    return keys

def add_key(c, k):
    for i in range(16):
        c[int(i / 4)][i % 4] ^= k[int(i / 4)][i % 4]

def byte_sub(c):
    for i in range(16):
        c[int(i / 4)][i % 4] = sbox[c[int(i / 4)][i % 4]]

def shift_rows(c):
    tmp = copy.deepcopy(c)
    for i in range(4):
        for j in range(1, 4):
            tmp[i][j] = c[(i + j) % 4][j]
    return tmp

def mix_col(c):
    tmp = copy.deepcopy(c)
    for i in range(4):
        for j in range(4):
            tmp[i][j] = xtime(c[i][j]) ^ xtime(c[i][(j+1)%4]) ^ c[i][(j+1)%4] ^ c[i][(j+2)%4] ^ c[i][(j+3)%4]
    return tmp

def encrypt(cipher, key):
    keys = gen_keys(num2mat(key))

    cipher = num2mat(plaintext)

    add_key(cipher, keys[0])

    for encrypt_round in range(1,10):
        byte_sub(cipher)
        cipher = shift_rows(cipher)
        cipher = mix_col(cipher)
        add_key(cipher, keys[encrypt_round])

    byte_sub(cipher)
    cipher = shift_rows(cipher)
    add_key(cipher, keys[10])

    return mat2num(cipher)

# Main
plaintext = 0x3243f6a8885a308d313198a2e0370734
key = 0x2b7e151628aed2a6abf7158809cf4f3c  # 0x5468617473206D79204B756E67204675
print(hex(encrypt(plaintext, key)))
