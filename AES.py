...
def replace_bytes(array: bytearray, first: int, second: int, size: int):
    ...
    array[first:first + size] = array[second:second + size]
    array[second:second + size] = tmp
...
for i in range(10):
    x = np.asarray(list(bin(2 ** i)[2:]), dtype=float)
    num = 0
    for indx, bit in enumerate(reversed(np.polydiv(x, aes_prime)[1])):
        if bit:
            num += 2 ** indx
    rc.append(num)
